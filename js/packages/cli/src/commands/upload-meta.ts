...
import { arweaveUploadMeta } from '../helpers/upload/arweave-meta';
...
export async function uploadMeta(
  ...
  files.forEach(f => {
    if (!seen[f.replace(EXTENSION_JSON, '').split('/').pop()]) {
      seen[f.replace(EXTENSION_JSON, '').split('/').pop()] = true;
      newFiles.push(f);
    }
  });
  existingInCache.forEach(f => {
    if (!seen[f]) {
      seen[f] = true;
      newFiles.push(f + '.json');
    }
  });

  const images = newFiles.filter(val => path.extname(val) === EXTENSION_PNG);
  const metadatas = newFiles.filter(
    val => path.extname(val) === EXTENSION_JSON,
  );
  ...
  for (let i = 0; i < SIZE; i++) {
    ...
    if (!link || !cacheContent.program.uuid) {
      ...
      const manifestContent = fs.readFileSync(metadata).toString();
